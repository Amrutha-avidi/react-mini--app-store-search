...
class AppStore extends Component {
  ...
  getApp = event => {
    this.setState({searchApp: event.target.value})
  }

  getSearchedApps = () => {
    const {searchApp} = this.state
    const filteredApps = appsList.filter(each =>
      each.appName.toLowerCase().includes(searchApp.toLowerCase()),
    )
    return filteredApps
  }
  ...
  render() {
    const {searchApp, activeTabId} = this.state
    const filteredApps = this.getSearchedApps()
    const filteredTabApps = this.getTabApps(filteredApps)
